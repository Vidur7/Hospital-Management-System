...
var con = mysql.createConnection({
  ...
  password: 'Chelsea@15', // MySQL password
  database: 'hms_table' // Name of the database you want to connect to
});
...
con.connect(function(err) {
  ...
  // Perform MySQL queries or other operations here
...
app.get('/makeAccount', (req, res) => {
  let query = req.query;
  let name = query.FName + " " + query.LName;
  ...
  let sql_statement = `INSERT INTO patients (FName, LName, email, password, gender) 
...
app.get('/makeDocAccount', (req, res) => {
  let params = req.query;
  let name = params.FName + " " + params.LName;
  let email = params.email;
  let password = params.password;
  let gender = params.gender;
  let schedule = params.schedule;
  let sql_statement = `INSERT INTO Doctor (email, gender, password, FName) 
                       VALUES ` + `("${email}", "${gender}", "${password}", "${FName}")`;
  console.log(sql_statement);
  con.query(sql_statement, function (error, results, fields) {
    if (error) throw error;
    else {
